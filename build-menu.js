...
\f0\fs24 \cf0 // build-menu.js (AGGIORNATO CON IL NUOVO BASE ID)\
\
const fetch = require('node-fetch');\
const fs = require('fs');\
\
// --- Configurazione Airtable (CON I TUOI NUOVI DATI) ---\
const AIRTABLE_BASE_ID = 'appGnw9kTGPSj9F59'; // <-- NUOVO ID BASE INSERITO QUI\
...
\};\
const configRecordId = 'recK0pTqrdvJWLi9d'; // Se anche questo ID \'e8 cambiato, dovrai aggiornarlo\
\
...
function generateMenuHTML(menuData) \{\
    if (!menuData || menuData.length === 0) \{\
        return '<p>Il menu non \'e8 disponibile al momento.</p>';\
    \}\
    let menuHTML = '';\
    menuData.forEach(category => \{\
        ...
        category.items.forEach(item => \{\
            let formattedPrice = ''; const priceValue = item.price; if (typeof priceValue === 'number') \{ formattedPrice = `\'80$\{priceValue.toFixed(2)\}`; \} else if (typeof priceValue === 'string') \{ formattedPrice = priceValue; \}\
...
\
    if (!AIRTABLE_PAT) \{\
        throw new Error("Errore: La chiave API AIRTABLE_PAT_KEY non \'e8 stata impostata nelle variabili d'ambiente!");\
    \}\
\
    try \{\
